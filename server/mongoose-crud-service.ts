...
export class MongooseCrudService<T> extends CrudService<T> {
  ...
  buildQuery(req: CrudRequest) {
    ...
    const options = {
      ...
      skip,
      limit,
      sort: sort.reduce((acc: Record<string, number>, {order, field}) => (acc[field] = order === 'ASC' ? 1 : -1, acc), {}),
      select: fields.join(' ')
    }
    const where = filter.reduce((acc: Record<string, object | null>, { field, operator, value }) => {
      let cond = null
      switch (operator) {
        case 'starts':
    ...
    return { options, where, id: idParam ? idParam.value : null, populate: join.map(v => v.field), }
  }
